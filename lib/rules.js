...
const getBuiltIn = node_fs_1.default
    .readdirSync(node_path_1.default.join(node_path_1.default.resolve(), '../../eslint/lib/rules'))
    .filter((builtIn) => builtIn.includes('.js'));
for (const builtIn of getBuiltIn) {
    importedBuiltIns.push(Promise.resolve().then(() => __importStar(require(node_path_1.default.join(node_path_1.default.resolve(), '../../eslint/lib/rules/', builtIn)))));
}
...
const getPlugins = node_fs_1.default
    .readdirSync(node_path_1.default.join(node_path_1.default.resolve(), '../../'))
...
for (const plugin of getPlugins) {
    ...
    if (plugin.includes('@')) {
        const pluginDirectory = node_fs_1.default
            .readdirSync(node_path_1.default.join(node_path_1.default.resolve(), '../../', plugin))
            .find((read) => /plugin/u.test(read));
        if (pluginDirectory) {
            copyIt = node_path_1.default.join(node_path_1.default.resolve(), '../../', plugin, pluginDirectory);
        }
    }
